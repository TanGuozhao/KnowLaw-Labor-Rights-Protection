...
const {
  ...
  addEvidenceOpenBtn,
  newEvidenceAutoCard,
  newEvidenceChoiceDialog,
  newEvidenceChoiceDialogClose,
  newEvidenceManualCard,
  newEvidenceOpenBtn,
  ...
  evidenceFileName,
  evidenceDetailBody,
  evidenceDetailCloseBtn,
  evidenceDetailDialog,
  evidenceDetailImportSourceBtn,
  evidenceDetailSubtitle,
  evidenceDetailViewSourceBtn,
  evidenceListEl,
  evidenceNoCaseHint,
  evidenceOcrBtn,
  evidenceOcrStatus,
...
let evidenceDetailTargetId = "";
let evidenceDetailFiles = [];
...
function renderEvidenceList(items) {
  ...
  evidenceListEl.innerHTML = list
    .map((it) => {
      ...
      const hasFile = Number(it.attachment_count || 0) > 0 || Boolean(String(it.file_path || "").trim());
      ...
      return `<div class="my-cases-evidence-card" data-evidence-card-id="${evidenceId}" role="button" tabindex="0" aria-label="查看证据详情">
...
const evidenceAutoCreateInput = document.createElement("input");
evidenceAutoCreateInput.type = "file";
evidenceAutoCreateInput.accept = evidenceCardUploadInput.accept;
evidenceAutoCreateInput.hidden = true;

const evidenceDetailImportInput = document.createElement("input");
evidenceDetailImportInput.type = "file";
evidenceDetailImportInput.accept = evidenceCardUploadInput.accept;
evidenceDetailImportInput.hidden = true;

async function openEvidenceDetailDialog(evidenceId) {
  const item = evidenceCache.find((it) => String(it?.evidence_id || "") === evidenceId);
  if (!item || !evidenceDetailDialog || !evidenceDetailBody) return;
  evidenceDetailTargetId = evidenceId;
  evidenceDetailFiles = [];
  try {
    const data = await listEvidenceFiles(evidenceId);
    evidenceDetailFiles = Array.isArray(data?.files) ? data.files : [];
  } catch {
    evidenceDetailFiles = [];
  }
  const fileRows = evidenceDetailFiles.length
    ? evidenceDetailFiles
      .map((f) => {
        const fid = escapeHtml(String(f.file_id || ""));
        const name = escapeHtml(String(f.original_filename || f.file_path || "附件"));
        const tag = f.is_primary ? "（当前主附件）" : "";
        const uploadedAt = escapeHtml(String(f.uploaded_at || "未知时间"));
        const sizeText = escapeHtml(formatFileSize(f.file_size));
        return `<div class="my-cases-evidence-detail-file-row">
          <button type="button" class="my-cases-evidence-file-link" data-detail-file-id="${fid}">${name}${tag}</button>
          <div class="my-cases-evidence-card-meta">上传时间：${uploadedAt} ｜ 文件大小：${sizeText}</div>
        </div>`;
      })
      .join("")
    : "—";
  if (evidenceDetailSubtitle) {
    evidenceDetailSubtitle.textContent = `${String(item.name || "未命名证据")}（${String(item.evidence_type || "未分类")}）`;
  }
  evidenceDetailBody.innerHTML = `<dl>
      <dt>证据名称</dt><dd>${escapeHtml(String(item.name || "—"))}</dd>
      <dt>证据类型</dt><dd>${escapeHtml(String(item.evidence_type || "—"))}</dd>
      <dt>产生时间</dt><dd>${escapeHtml(String(item.related_time || "—"))}</dd>
      <dt>来源</dt><dd>${escapeHtml(String(item.source || "—"))}</dd>
      <dt>描述</dt><dd>${escapeHtml(String(item.description || "—"))}</dd>
      <dt>附件列表</dt><dd>${fileRows}</dd>
      <dt>解析文本</dt><dd>${escapeHtml(String(item.ocr_text || "（暂无解析文本）"))}</dd>
    </dl>`;
  evidenceDetailDialog.showModal();
}

async function openEvidenceFileById(evidenceId, fileId = "") {
  const { blob, contentType } = fileId
    ? await fetchEvidenceFileBlobByFileId(evidenceId, fileId)
    : await fetchEvidenceFileBlob(evidenceId);
...
export function initEvidencePanel() {
  ...
  if (!evidenceCardUploadInput.isConnected && document.body) {
    ...
    document.body.appendChild(evidenceAutoCreateInput);
    document.body.appendChild(evidenceDetailImportInput);
  }
  ...
  if (addEvidenceOpenBtn) {
    addEvidenceOpenBtn.addEventListener("click", () => openAddEvidenceDialog());
  }
  if (newEvidenceOpenBtn) {
    newEvidenceOpenBtn.addEventListener("click", () => {
      syncAddEvidenceButton();
      if (newEvidenceOpenBtn.disabled) {
        setEvidenceStatus("请先在「案件档案」中创建并选中当前维权案件。", true);
        return;
      }
      newEvidenceChoiceDialog?.showModal();
    });
  }
  if (newEvidenceChoiceDialogClose) {
    newEvidenceChoiceDialogClose.addEventListener("click", () => newEvidenceChoiceDialog?.close());
  }
  if (newEvidenceManualCard) {
    newEvidenceManualCard.addEventListener("click", () => {
      newEvidenceChoiceDialog?.close();
      openAddEvidenceDialog();
    });
  }
  if (newEvidenceAutoCard) {
    newEvidenceAutoCard.addEventListener("click", () => {
      newEvidenceChoiceDialog?.close();
      evidenceAutoCreateInput.value = "";
      evidenceAutoCreateInput.click();
    });
  }
  ...
  if (addEvidenceFileInput) {
    addEvidenceFileInput.addEventListener("change", async () => {
      ...
      try {
        ...
        setAddEvidenceUploadStatus("已自动入库。", false);
  ...
  if (evidenceListEl) {
    evidenceListEl.addEventListener("click", (event) => {
      const card = event.target instanceof Element ? event.target.closest("[data-evidence-card-id]") : null;
      const btn = event.target instanceof Element ? event.target.closest("button[data-action][data-evidence-id]") : null;
      if (!btn && card) {
        const evidenceId = String(card.getAttribute("data-evidence-card-id") || "").trim();
        if (evidenceId) void openEvidenceDetailDialog(evidenceId);
        return;
      }
      if (!btn) return;
  ...
  evidenceAutoCreateInput.addEventListener("change", async () => {
    const file = evidenceAutoCreateInput.files?.[0];
    const cid = getSelectedCaseId();
    if (!file || !cid) return;
    try {
      setEvidenceStatus("正在自动解析并创建证据档案…");
      await uploadEvidenceAnalyze(file, cid);
      setEvidenceStatus("已创建证据档案");
      newEvidenceChoiceDialog?.close();
      await loadEvidenceSummary();
    } catch (err) {
      setEvidenceStatus(err?.message || "自动解析失败", true);
    } finally {
      evidenceAutoCreateInput.value = "";
    }
  });

  if (evidenceDetailCloseBtn) {
    evidenceDetailCloseBtn.addEventListener("click", () => evidenceDetailDialog?.close());
  }
  if (evidenceDetailViewSourceBtn) {
    evidenceDetailViewSourceBtn.addEventListener("click", () => {
      if (!evidenceDetailTargetId) return;
      void openEvidenceFileById(evidenceDetailTargetId).catch((err) => {
        setEvidenceStatus(err?.message || "查看失败", true);
      });
    });
  }
  if (evidenceDetailBody) {
    evidenceDetailBody.addEventListener("click", (event) => {
      const btn = event.target instanceof Element ? event.target.closest("[data-detail-file-id]") : null;
      if (!btn || !evidenceDetailTargetId) return;
      const fileId = String(btn.getAttribute("data-detail-file-id") || "").trim();
      if (!fileId) return;
      void openEvidenceFileById(evidenceDetailTargetId, fileId).catch((err) => {
        setEvidenceStatus(err?.message || "查看失败", true);
      });
    });
  }
  if (evidenceDetailImportSourceBtn) {
    evidenceDetailImportSourceBtn.addEventListener("click", () => {
      if (!evidenceDetailTargetId) return;
      evidenceDetailImportInput.value = "";
      evidenceDetailImportInput.click();
    });
  }
  evidenceDetailImportInput.addEventListener("change", async () => {
    const file = evidenceDetailImportInput.files?.[0];
    if (!file || !evidenceDetailTargetId) return;
    try {
      setEvidenceStatus("正在导入源文件并自动解析…");
      await reanalyzeEvidenceFile(evidenceDetailTargetId, file);
      setEvidenceStatus("证据已更新并自动解析");
      await loadEvidenceSummary();
      void openEvidenceDetailDialog(evidenceDetailTargetId);
    } catch (err) {
      setEvidenceStatus(err?.message || "导入失败", true);
    } finally {
      evidenceDetailImportInput.value = "";
    }
  });
