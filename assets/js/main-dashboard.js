...
function formatDocumentHeading(value) {
...
function splitCaseContentLines(value) {
  const raw = String(value ?? "").trim();
  if (!raw) return [];
  const normalized = normalizeEmArtifacts(raw)
    .replace(/\r\n?/g, "\n")
    .replace(/<\s*br\s*\/?\s*>/gi, "\n")
    .replace(/<\s*hr[^>]*>/gi, "\n")
...
function formatCaseParagraphHtml(rawLine) {
  ...
  if (!line) return "";
  /* 与独立类案详情一致：不按「某某：」拆成标签+正文，避免「法院认为：」等显示为加粗标题 */
  return `<p class="case-detail-paragraph case-detail-paragraph--body">${toRichTextHtml(line)}</p>`;
}

function buildCaseContentHtml(value) {
  const raw = String(value ?? "").trim();
  if (!raw) {
    return `<p class="case-detail-empty-copy">暂无详细内容</p>`;
  }

  if (/<(?:table|thead|tbody|tfoot|tr|td|th|ul|ol|dl|blockquote)\b/i.test(raw)) {
    const structuredHtml = sanitizeRichHtml(raw);
    if (structuredHtml.trim()) {
      return `<div class="case-detail-rich case-detail-rich--structured">${structuredHtml}</div>`;
    }
  }

  const lines = splitCaseContentLines(raw);
  if (!lines.length) {
    const fallbackHtml = toRichTextHtml(raw);
    return fallbackHtml
      ? `<div class="case-detail-rich">${fallbackHtml}</div>`
      : `<p class="case-detail-empty-copy">暂无详细内容</p>`;
  }

  const paragraphs = lines.map((line) => formatCaseParagraphHtml(String(line || "").trim())).join("");
  return `
    <section class="case-detail-section case-detail-section--plain">
      <div class="case-detail-section-body">${paragraphs}</div>
    </section>
  `;
}

function buildCaseMetaGrid(entries) {
  const validEntries = (entries || []).filter((entry) => entry?.value);
  if (!validEntries.length) return "";
  return `
    <dl class="case-detail-meta-grid">
      ${validEntries
        .map(
          (entry) => `
            <div class="case-detail-meta-item">
              <dt class="case-detail-meta-label">${escapeHtml(entry.label || "")}</dt>
              <dd class="case-detail-meta-value">${entry.value}</dd>
            </div>
          `
        )
        .join("")}
...
function renderCaseDetail(item) {
  ...
  const rawTitle = firstNonEmptyText(item?.title, item?.caseName, item?.name, item?.docTitle);
  const rawCourt = firstNonEmptyText(item?.court, item?.courtName);
  const rawDate = firstNonEmptyText(item?.judgementDate, item?.judgementTime, item?.date);
  const rawCaseNo = firstNonEmptyText(item?.caseNumber, item?.caseNo, item?.docNo);
  const rawCaseType = firstNonEmptyText(item?.caseType, item?.docType, item?.documentType);
  const rawLevel = firstNonEmptyText(item?.levelOfTrial);
  const rawContent = firstNonEmptyText(
    item?.content,
    item?.detail,
    item?.detailContent,
    item?.htmlContent,
  ...
  const title = toRichTextHtml(rawTitle || "（未命名案例）");
  const court = toRichTextHtml(rawCourt);
  const date = toRichTextHtml(rawDate);
  const caseNo = toRichTextHtml(rawCaseNo);
  const caseType = toRichTextHtml(rawCaseType);
  const level = toRichTextHtml(rawLevel);
  const documentType = deriveCaseDocumentType(rawTitle, rawCaseType);
  const documentHeading = formatDocumentHeading(documentType);
  const metaGrid = buildCaseMetaGrid([
    { label: "法院", value: court },
    { label: "日期", value: date },
    { label: "案号", value: caseNo },
    { label: "类型", value: caseType },
    { label: "审级", value: level },
  ]);
  const contentHtml = buildCaseContentHtml(rawContent);

  if (!caseDetail) return;
  caseDetail.innerHTML = `
    <article class="case-detail-sheet">
      <header class="case-detail-sheet-head">
        <h3 class="case-detail-page-title">${title}</h3>
        ${metaGrid}
      </header>
      <div class="case-detail-divider"></div>
      <section class="case-detail-doc">
        <div class="case-detail-doc-head">
          ${court ? `<p class="case-detail-doc-court">${court}</p>` : ""}
          ${documentHeading ? `<h4 class="case-detail-doc-type">${escapeHtml(documentHeading)}</h4>` : ""}
          ${caseNo ? `<p class="case-detail-doc-number">${caseNo}</p>` : ""}
        </div>
        <div class="case-detail-doc-body">${contentHtml}</div>
      </section>
    </article>
  `;
...
function renderLawDetail(item) {
  ...
  if (!caseDetail) return;
  caseDetail.innerHTML = `
    <article class="case-detail-sheet case-detail-sheet--law">
      <header class="case-detail-sheet-head">
        <h3 class="case-detail-page-title">法律条文详情</h3>
      </header>
      <div class="case-detail-divider"></div>
      ${
        textHtml
          ? `<div class="law-detail-text case-detail-rich case-detail-rich--structured">${textHtml}</div>`
          : `<p class="case-detail-empty-copy">暂无条文内容</p>`
      }
    </article>
  `;
  caseDetail.scrollTop = 0;
