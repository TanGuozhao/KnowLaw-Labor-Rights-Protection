...
const medOrgEl = document.getElementById("medOrg");
const medClaimsEl = document.getElementById("medClaims");
const medFactsEl = document.getElementById("medFacts");
const medEvidenceListEl = document.getElementById("medEvidenceList");
...
function collectLaborMediationFormData() {
  return {
    ...
    mediation_org: String(medOrgEl?.value || "").trim(),
    claims: String(medClaimsEl?.value || "").trim(),
    facts: String(medFactsEl?.value || "").trim(),
    evidence_list: String(medEvidenceListEl?.value || "").trim(),
    dateText: nowDateText(),
  };
}
...
function formatLaborApplicantBlock(p) {
  const parts = [
    ...
    p.applicant_id_number && `公民身份号码：${p.applicant_id_number}`,
...
function renderLaborArbitrationHtml(payload) {
  ...
  const applicantLine = formatLaborApplicantBlock(payload);
  const respondentLine = formatLaborRespondentBlock(payload);
  ...
  return `<article class="doc-gen-civil-wrap" aria-label="劳动仲裁申请书预览">
  <h2 class="doc-gen-civil-title">${escapeHtml(getDocTypeLabel("labor_arbitration_application"))}</h2>
  <p class="doc-gen-civil-p"><strong>申请人：</strong>${applicantLine ? escapeHtml(applicantLine) : v("")}</p>
  <p class="doc-gen-civil-p"><strong>被申请人：</strong>${respondentLine ? escapeHtml(respondentLine) : v("")}</p>
  <div class="doc-gen-civil-section">
    <h3 class="doc-gen-civil-h3">仲裁请求</h3>
  ...
  <div class="doc-gen-civil-section">
    <h3 class="doc-gen-civil-h3">证据和证据来源（如有）</h3>
    <div class="doc-gen-civil-body">${evHtml || v("（无）")}</div>
  </div>
  <p class="doc-gen-civil-p doc-gen-civil-signoff"><strong>此致</strong></p>
  <p class="doc-gen-civil-p doc-gen-civil-court">${v(payload.arbitration_commission)}</p>
  <p class="doc-gen-civil-p doc-gen-civil-sign"><strong>申请人：</strong>${v(payload.applicant_name)}（签名）</p>
  <p class="doc-gen-civil-p doc-gen-civil-date">${v(payload.dateText)}</p>
</article>`;
}
...
function renderLaborMediationHtml(payload) {
  ...
  const factsHtml = formatFactsSectionHtml(payload.facts, "");
  const evHtml = formatFactsSectionHtml(payload.evidence_list, "");
  return `<article class="doc-gen-civil-wrap" aria-label="劳动争议调解申请书预览">
  <h2 class="doc-gen-civil-title">${escapeHtml(getDocTypeLabel("labor_mediation_application"))}</h2>
  <p class="doc-gen-civil-p"><strong>申请人：</strong>${applicantLine ? escapeHtml(applicantLine) : v("")}</p>
  <p class="doc-gen-civil-p"><strong>被申请人：</strong>${respondentLine ? escapeHtml(respondentLine) : v("")}</p>
  <div class="doc-gen-civil-section">
    <h3 class="doc-gen-civil-h3">调解请求</h3>
    <div class="doc-gen-civil-body">${claimsHtml}</div>
  </div>
  <div class="doc-gen-civil-section">
    <h3 class="doc-gen-civil-h3">事实与理由</h3>
  ...
  <p class="doc-gen-civil-p doc-gen-civil-signoff"><strong>此致</strong></p>
  <p class="doc-gen-civil-p doc-gen-civil-court">${v(payload.mediation_org)}</p>
  <p class="doc-gen-civil-p doc-gen-civil-sign"><strong>申请人：</strong>${v(payload.applicant_name)}（签名）</p>
  <p class="doc-gen-civil-p doc-gen-civil-date">${v(payload.dateText)}</p>
</article>`;
}
...
function fillForm(data) {
  ...
  if (!inferredTemplate) {
    ...
    else if (String(data.mediation_org || "").trim()) inferredTemplate = "labor_mediation_application";
  ...
  if (inferredTemplate === "labor_mediation_application") {
    ...
    if (medOrgEl) medOrgEl.value = data.mediation_org || "";
    if (medClaimsEl) medClaimsEl.value = data.claims || "";
    if (medFactsEl) medFactsEl.value = data.facts || "";
    if (medEvidenceListEl) medEvidenceListEl.value = data.evidence_list || "";
    return;
  }
...
async function applySelectedCase(rawCaseId) {
  ...
  setStatus("已从案件载入表单，请核对后可直接保存 Word 或 PDF。");
  if (
