...
function renderList() {
  ...
  els.list.innerHTML = cases
    .map((item) => {
      ...
      return `
        <button type="button" class="rm-card${active}" data-id="${id}">
          <div class="rm-card-title">${title}</div>
          <div class="rm-card-meta">${respondent}</div>
        </button>
      `;
    })
    .join("");
...
function renderDetail() {
  ...
  const field = (label, value) => `
    <div class="rm-field">
      <div class="rm-field-label">${escapeHtml(label)}</div>
      <div class="rm-field-value">${escapeHtml(value || "未填写")}</div>
    </div>
  `;
  els.detail.innerHTML = `
    <h2 class="rm-title">${escapeHtml(current.title || current.reason || "维权事项")}</h2>
    <div class="rm-detail-grid">
      ${field("被申请人", current.respondent_name)}
      ${field("案由", current.reason)}
      ${field("当前阶段", current.stage)}
      ${field("创建时间", current.build_time)}
      ${field("诉求摘要", current.request)}
      ${field("经过说明", current.details)}
    </div>
  `;
}
...
async function init() {
  ...
  try {
    ...
    if (els.detail) {
      els.detail.innerHTML = `<div class="rm-empty">${escapeHtml(error?.message || "加载失败")}</div>`;
    }
  }
}
